//=================================================================================================
/*!
 *  \file src/core/rigidbody/Ellipsoid.cpp
 *  \brief Source file for the Ellipsoid class
 *
...
//*************************************************************************************************
/*!\brief Instantiation constructor for the Ellipsoid class.
 *
 * \param sid Unique system-specific ID for the sphere.
 * \param uid User-specific ID for the sphere.
 * \param gpos Global geometric center of the sphere.
 ...
 */
void Ellipsoid::setVisible( bool visible )
{
   if( visible ^ visible_ ) {
      visible_ = visible;
 ...
 */
void Ellipsoid::setPosition( real px, real py, real pz )
{
   ...
   Ellipsoid::calcBoundingBox();    // Updating the axis-aligned bounding box of the sphere
 ...
 *   in a \a std::logic_error.
 */
void Ellipsoid::setPosition( const Vec3& gpos )
{
   if( global_ && ExclusiveSection::isActive() )
      throw std::logic_error( "Invalid translation of a global sphere inside an exclusive section" );
 ...
 */
void Ellipsoid::setOrientation( real r, real i, real j, real k )
{
   if( global_ && ExclusiveSection::isActive() )
 ...
 * neglected and overwritten by the settings of the rigid body on its local process!
 */
void Ellipsoid::setOrientation( const Quat& q )
{
   if( global_ && ExclusiveSection::isActive() )
      throw std::logic_error( "Invalid rotation of a global sphere inside an exclusive section" );
 ...
 *   a \a std::logic_error.
 */
void Ellipsoid::translate( real dx, real dy, real dz )
{
   if( global_ && ExclusiveSection::isActive() )
      throw std::logic_error( "Invalid translation of a global sphere inside an exclusive section" );
   ...
   Ellipsoid::calcBoundingBox();    // Updating the axis-aligned bounding box
   wake();               // Waking the sphere from sleep mode
   signalTranslation();  // Signaling the position change to the superordinate body
}
//*************************************************************************************************
 ...
 *   a \a std::logic_error.
 */
void Ellipsoid::translate( const Vec3& dp )
{
   if( global_ && ExclusiveSection::isActive() )
      throw std::logic_error( "Invalid translation of a global sphere inside an exclusive section" );
 ...
 *   in a \a std::logic_error.
 */
void Ellipsoid::rotate( real x, real y, real z, real angle )
{
 ...
 *   in a \a std::logic_error.
 */
void Ellipsoid::rotate( const Vec3& axis, real angle )
{
   if( global_ && ExclusiveSection::isActive() )
      throw std::logic_error( "Invalid rotation of a global sphere inside an exclusive section" );
 ...
 *   in a \a std::logic_error.
 */
void Ellipsoid::rotate( real xangle, real yangle, real zangle )
{
   if( global_ && ExclusiveSection::isActive() )
      throw std::logic_error( "Invalid rotation of a global sphere inside an exclusive section" );
 ...
 *   in a \a std::logic_error.
 */
void Ellipsoid::rotate( const Vec3& euler )
{
 ...
 *   in a \a std::logic_error.
 */
void Ellipsoid::rotate( const Quat& dq )
{
   if( global_ && ExclusiveSection::isActive() )
      throw std::logic_error( "Invalid rotation of a global sphere inside an exclusive section" );
 ...
 *   in a \a std::logic_error.
 */
void Ellipsoid::rotateAroundOrigin( real x, real y, real z, real angle )
{
 ...
 *   in a \a std::logic_error.
 */
void Ellipsoid::rotateAroundOrigin( const Vec3& axis, real angle )
{
   if( global_ && ExclusiveSection::isActive() )
      throw std::logic_error( "Invalid rotation of a global sphere inside an exclusive section" );
 ...
 *   in a \a std::logic_error.
 */
void Ellipsoid::rotateAroundOrigin( real xangle, real yangle, real zangle )
{
   if( global_ && ExclusiveSection::isActive() )
      throw std::logic_error( "Invalid rotation of a global sphere inside an exclusive section" );
 ...
 *   in a \a std::logic_error.
 */
void Ellipsoid::rotateAroundOrigin( const Vec3& euler )
{
 ...
 *   in a \a std::logic_error.
 */
void Ellipsoid::rotateAroundPoint( const Vec3& point, const Vec3& axis, real angle )
{
   if( global_ && ExclusiveSection::isActive() )
      throw std::logic_error( "Invalid rotation of a global sphere inside an exclusive section" );
 ...
 *   in a \a std::logic_error.
 */
void Ellipsoid::rotateAroundPoint( const Vec3& point, const Vec3& euler )
{
   if( global_ && ExclusiveSection::isActive() )
      throw std::logic_error( "Invalid rotation of a global sphere inside an exclusive section" );
...
bool Ellipsoid::containsPoint( real px, real py, real pz ) const
{
   ...
   if( (px * px / radiusA_ * radiusA_ + py * py / radiusB_ * radiusB_ + pz * pz / radiusC_ * radiusC_) <= 1.0) {
      return true;
   }
   else {
      return false;
 ...
 * bounding box.
 */
void Ellipsoid::update( const Vec3& dp )
{
 ...
 */
void Ellipsoid::update( const Quat& dq )
{
   // Checking the state of the sphere
   pe_INTERNAL_ASSERT( checkInvariants(), "Invalid sphere state detected" );
   pe_INTERNAL_ASSERT( hasSuperBody(), "Invalid superordinate body detected" );
...
void Ellipsoid::print( std::ostream& os, const char* tab ) const
{
   ...
   os << tab << " Ellipsoid " << uid_ << " with radius " << radiusA_ << "\n";
 ...
 * The following code example illustrates the setup of a sphere:
   ...
   EllipsoidID sphere = createEllipsoid( 1, Vec3(2,3,4), 2.5, iron );
 ...
 * added to the newly created union:
   ...
   pe_CREATE_UNION( newunion, 1 )
   {
      ...
      // part of the union.
      createEllipsoid( 2, Vec3(-1,4,-5), 1.3, iron );
      ...
   }
   \endcode
