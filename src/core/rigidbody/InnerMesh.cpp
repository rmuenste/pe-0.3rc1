//=================================================================================================
/*!
 *  \file src/core/rigidbody/InnerMesh.cpp
...
#include <pe/core/rigidbody/InnerMesh.h>
...
//*************************************************************************************************
/*!\brief Constructor for the InnerMesh class
 *
 ...
 */
InnerMesh::InnerMesh( id_t sid, id_t uid, const Vec3& gpos,
...
//*************************************************************************************************
/*!\brief Destructor for the InnerMesh class.
 */
InnerMesh::~InnerMesh()
{
...
void InnerMesh::initSTL( const char* const file,
                            ...
                            Normals& faceNormals)
{
   ...
   if(vertices.size() % 3 != 0) {
      ...
      error << "InnerMesh::initSTL: Corrupted input file, number of vertices is not a multiple of 3.\n";
   ...
   if(vertices.size() / 3 != faceNormals.size()) {
      std::stringstream error;
      error << "InnerMesh::initSTL: Corrupted input file, number of vertices is not appropriate to the number of faces.\n";
      pe_LOG_INFO_SECTION(log) {
         log << error.str();
      }
      throw std::invalid_argument(error.str());
 ...
 * \todo Review documentation.
 */
void InnerMesh::initOBJ(const std::string file,
                  Vertices& vertices, IndicesLists& faceIndices,
                  Normals& faceNormals,
                  ...
                  bool clockwise, bool lefthanded)
{
   ...
   if(faceIndices.size()<4){
      std::stringstream error;
      error << "InnerMesh::initOBJ: TriangleMashes with less than 4 (" << faceIndices.size() << ") faces not allowed.\n";
      pe_LOG_INFO_SECTION(log) {
         log << error.str();
      }
      throw std::invalid_argument(error.str());
   ...
   for(size_t f = 0; f < faceIndices.size(); ++f) {
      ...
      if(normalIndices.size() > 0) {
         ...
         if(cosVec1 < 0 && cosVec2 < 0 && cosVec3 < 0 && !inverted) {
            std::stringstream error;
            error << "InnerMesh::initOBJ: Vertex order seams to be inverted.\n";
            pe_LOG_INFO_SECTION(log) {
               log << error.str();
            }
            inverted = true;
            ...
            std::stringstream error;
            error << "InnerMesh::initOBJ: At least one vertex normal does not match face normal direction.\n";
            pe_LOG_INFO_SECTION(log) {
               log << error.str();
            }
         }
   ...
   for( size_t v = 0; v < vertexVNeighbor_.size(); ++v ) {
      ...
      for( size_t n = 0; n < verticesOriginal_.size(); ++n ) {
         ...
         if( vertexEdge_[neighborIndex] == faceIndices_.size()*3 ) {
            stringstream error;
            error << "InnerMesh::initHalfEdge: InnerMesh(sid="<< this->getID()<<") vertex with index="<<
 ...
 */
void InnerMesh::setVisible( bool visible )
{
   if( visible ^ visible_ ) {
      visible_ = visible;
 ...
 * \todo Review documentation.
 */
void InnerMesh::setRenderSmooth( bool smooth) 
{
 ...
 */
void InnerMesh::setPosition( real px, real py, real pz )
{
 ...
 *   in a \a std::logic_error.
 */
void InnerMesh::setPosition( const Vec3& gpos )
{
   if( global_ && ExclusiveSection::isActive() )
      throw std::logic_error( "Invalid translation of a global triangle mesh inside an exclusive section" );
   ...
   InnerMesh::calcBoundingBox();    // Updating the axis-aligned bounding box of the triangle mesh
   wake();               // Waking the triangle mesh from sleep mode
   signalTranslation();  // Signaling the position change to the superordinate body
}
//*************************************************************************************************
 ...
 *   in a \a std::logic_error.
 */
void InnerMesh::setOrientation( real r, real i, real j, real k )
{
   if( global_ && ExclusiveSection::isActive() )
 ...
 *   in a \a std::logic_error.
 */
void InnerMesh::setOrientation( const Quat& q )
{
   if( global_ && ExclusiveSection::isActive() )
      throw std::logic_error( "Invalid rotation of a global triangle mesh inside an exclusive section" );
 ...
 *   a \a std::logic_error.
 */
void InnerMesh::translate( real dx, real dy, real dz )
{
   if( global_ && ExclusiveSection::isActive() )
      throw std::logic_error( "Invalid translation of a global triangle mesh inside an exclusive section" );
 ...
 *   a \a std::logic_error.
 */
void InnerMesh::translate( const Vec3& dp )
{
   if( global_ && ExclusiveSection::isActive() )
      throw std::logic_error( "Invalid translation of a global triangle mesh inside an exclusive section" );
 ...
 *   in a \a std::logic_error.
 */
void InnerMesh::rotate( real x, real y, real z, real angle )
{
 ...
 *   in a \a std::logic_error.
 */
void InnerMesh::rotate( const Vec3& axis, real angle )
{
   if( global_ && ExclusiveSection::isActive() )
      throw std::logic_error( "Invalid rotation of a global triangle mesh inside an exclusive section" );
 ...
 *   in a \a std::logic_error.
 */
void InnerMesh::rotate( real xangle, real yangle, real zangle )
{
   if( global_ && ExclusiveSection::isActive() )
      throw std::logic_error( "Invalid rotation of a global triangle mesh inside an exclusive section" );
 ...
 *   in a \a std::logic_error.
 */
void InnerMesh::rotate( const Vec3& euler )
{
 ...
 *   in a \a std::logic_error.
 */
void InnerMesh::rotate( const Quat& dq )
{
   if( global_ && ExclusiveSection::isActive() )
      throw std::logic_error( "Invalid rotation of a global triangle mesh inside an exclusive section" );
 ...
 *   in a \a std::logic_error.
 */
void InnerMesh::rotateAroundOrigin( real x, real y, real z, real angle )
{
 ...
 *   in a \a std::logic_error.
 */
void InnerMesh::rotateAroundOrigin( const Vec3& axis, real angle )
{
   if( global_ && ExclusiveSection::isActive() )
      throw std::logic_error( "Invalid rotation of a global triangle mesh inside an exclusive section" );
 ...
 *   in a \a std::logic_error.
 */
void InnerMesh::rotateAroundOrigin( real xangle, real yangle, real zangle )
{
   if( global_ && ExclusiveSection::isActive() )
      throw std::logic_error( "Invalid rotation of a global triangle mesh inside an exclusive section" );
 ...
 *   in a \a std::logic_error.
 */
void InnerMesh::rotateAroundOrigin( const Vec3& euler )
{
 ...
 *   in a \a std::logic_error.
 */
void InnerMesh::rotateAroundPoint( const Vec3& point, const Vec3& axis, real angle )
{
   if( global_ && ExclusiveSection::isActive() )
      throw std::logic_error( "Invalid rotation of a global triangle mesh inside an exclusive section" );
 ...
 *   in a \a std::logic_error.
 */
void InnerMesh::rotateAroundPoint( const Vec3& point, const Vec3& euler )
{
   if( global_ && ExclusiveSection::isActive() )
      throw std::logic_error( "Invalid rotation of a global triangle mesh inside an exclusive section" );
 ...
 */
bool InnerMesh::containsRelPoint( real px, real py, real pz ) const
{
 ...
 * \todo Review documentation.
 */
struct InnerMesh::LowToHighComp
{
...
bool InnerMesh::containsRelPoint( const Vec3& rpos ) const
{
 ...
 * \return \a true if the point lies inside the triangle mesh, \a false if not.
 */
bool InnerMesh::containsPoint( real px, real py, real pz ) const
{
 ...
 * \return \a true if the point lies inside the triangle mesh, \a false if not.
 */
bool InnerMesh::containsPoint( const Vec3& gpos ) const
{
 ...
 */
bool InnerMesh::isSurfaceRelPoint( real px, real py, real pz ) const
{
 ...
 * The tolerance level of the check is pe::surfaceThreshold.
 */
bool InnerMesh::isSurfaceRelPoint( const Vec3& rpos ) const
{
 ...
 * The tolerance level of the check is pe::surfaceThreshold.
 */
bool InnerMesh::isSurfacePoint( real px, real py, real pz ) const
{
 ...
 * The tolerance level of the check is pe::surfaceThreshold.
 */
bool InnerMesh::isSurfacePoint( const Vec3& gpos ) const
{
 ...
 */
real InnerMesh::getRelDepth( real px, real py, real pz ) const
{
 ...
 * lies outside the triangle mesh. The returned depth is calculated relative to the closest side of the triangle mesh.
 */
real InnerMesh::getRelDepth( const Vec3& rpos ) const
{
 ...
 */
void InnerMesh::calcBoundingBox()
{
   //FEATURE? Eventuell entscheidung für kleine TM einbauen
   //FEATURE? eventuell einfach nur die InnerMeshBase:support Funktion benutzen
   if(!convex_) {
 ...
 * bounding box.
 */
void InnerMesh::update( const Vec3& dp )
{
 ...
 */
void InnerMesh::update( const Quat& dq )
{
   // Checking the state of the triangle mesh
   pe_INTERNAL_ASSERT( checkInvariants(), "Invalid triangle mesh state detected" );
   pe_INTERNAL_ASSERT( hasSuperBody(), "Invalid superordinate body detected" );
 ...
 * \todo Review documentation.
 */
bool InnerMesh::hasClosedSurface() const {
 ...
 * \todo Review documentation.
 */
Vec3 InnerMesh::support( const Vec3& d, size_t startIndex = 0, size_t* pointIndex = NULL) const
{
 ...
 * \todo Review documentation.
 */
void InnerMesh::setColoredTriangleTexture(povray::WriterID pov)
{
   ...
   pov->setTexture( static_cast<ConstInnerMeshID>(this), coloredTrianglesTexture);
   updatePOVcacheString();
}
//*************************************************************************************************
 ...
 * \todo Review documentation.
 */
void InnerMesh::print( std::ostream& os, const char* tab ) const
{
   using std::setw;
 ...
 * \todo Review documentation.
 */
void InnerMesh::printPOVmesh2(std::ostream& os, const povray::Texture& texture,
 ...
 *  \todo Review documentation.
 */
void InnerMesh::updatePOVcacheString()
{
   char const * delim = "";
 ...
 */
void InnerMesh::updateIrrlichtCacheBuffer()
{
   using ::irr::u16;
   using ::irr::f32;
 ...
 * \todo Review documentation.
 */
PE_PUBLIC InnerMeshID createInnerMesh( id_t uid, const Vec3& gpos, const std::string file,
                                     ...
                                     bool visible, const Vec3& scale,  bool clockwise, bool lefthanded )
{
   ...
   /*NOT supported any longer as there is no halfe endge informiateon
      ...
      InnerMesh::initSTL(file.c_str(), vertices, faceIndices, faceNormals);
   }
   */
   else {
...
*/
   ...
   InnerMeshID mesh = new InnerMesh( sid, uid, gpos,
            vertices, faceIndices,
            faceNormals,
            vertexNormals, normalIndices,
            texturCoordinates, texturIndices,
 ...
 */
PE_PUBLIC InnerMeshID createInnerMesh( id_t uid, const Vec3& gpos, const InnerMeshID source, 
                                     MaterialID material, bool visible)
{
   ...
   const id_t sid( global ? UniqueID<RigidBody>::createGlobal() : UniqueID<RigidBody>::create() );
   //Creating the new triangle mesh
   InnerMeshID copy = new InnerMesh( sid, uid, gpos,
 ...
 * \todo Document.
 */
PE_PUBLIC InnerMeshID createInnerMesh( id_t uid, const Vec3& gpos, Vertices vertices,
                                     ...
                                     bool convex, bool visible )
{
   ...
   // Creating the new triangle mesh
   const id_t sid( global ? UniqueID<RigidBody>::createGlobal() : UniqueID<RigidBody>::create() );
   InnerMeshID mesh = new InnerMesh( sid, uid, gpos, vertices, faces, Normals(), Normals(),
                                ...
                                bool fixed, bool reg )
{
   ...
   // Creating the new triangle mesh
   InnerMeshID mesh = new InnerMesh( sid, uid, gpos, rpos, q, vertices, faces, Normals(), Normals(),
                                   IndicesLists(), TextureCoordinates(), IndicesLists(), material,
                                   visible, fixed, true );
