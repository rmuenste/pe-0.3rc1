//=================================================================================================
/*!
 *  \file examples/mpibrazil/chull.cpp
 *  \brief Source file for computation of convex hulls
 *
 *  Copyright (C) 2000 Joseph O'Rourke
 *                2014 Tobias Preclik
 *
 ...
 *
 *  The code from this file is copied and modified from Computational Geometry in C (Second
 *  Edition) by Joseph O'Rourke, published by Cambridge University Press. The original author of
 *  the source code Joseph O'Rourke <orourke@cs.smith.edu> permitted the relicensing of the chull
 *  source under the GPL version 3 license as long as the attribution of the original authorship
 *  is retained. This is a license supplement in agreement with section 7 of the GPL version 3.
 */
//=================================================================================================
...
not removed.
--------------------------------------------------------------------
With the explicit consent of the author the above copyright notice
was relaxed by allowing source code modifications but still
requiring that the original authorship is properly attributed to
Joseph O'Rourke -- Tobias Preclik
--------------------------------------------------------------------
*/
...
struct tEdgeStructure {
   ...
   bool     del;                /* true iff edge should be deleted. */
   tEdge    next, prev;
};
...
{
   std::vector<int> vMap( n );
   tVertex  v;
   tEdge    e;
   tFace    f;

   pe_INTERNAL_ASSERT( vertices == NULL && faces == NULL && edges == NULL, "chull code is in dirty state on entry" );
   ...
   for( int i = 0; i < n; ++i ) {
      ...
      do {
         ...
         v->v[Y] = rand() % 100 - 50;
         v->v[Z] = rand() % 100 - 50;
      } while ( v->v[X] * v->v[X] + v->v[Y] * v->v[Y] + v->v[Z] * v->v[Z] > 50*50 );
      v->vnum = i;
   }
   ...
   // Count number of vertices in the convex hull and remap indices
   v = vertices;
   int vnumOut = 0;
   do {
      vMap[v->vnum] = vnumOut++;
   ...
   // Store rescaled vertices in the output vector
   v = vertices;
   pe::real scale( radius / pe::real( 50 ) );
   verticesOut.clear();
   verticesOut.resize( vnumOut );
   for( int i = 0; i < vnumOut; ++i ) {
      verticesOut[i] = pe::Vec3( v->v[X], v->v[Y], v->v[Z] ) * scale;
      v = v->next;
   }
   ...
   // Store faces in the output vector
   facesOut.clear();
   facesOut.resize( fnumOut );
   for( int i = 0; i < fnumOut; ++i ) {
      facesOut[i] = pe::Vector3<size_t>( vMap[f->vertex[0]->vnum], vMap[f->vertex[1]->vnum], vMap[f->vertex[2]->vnum] );
      //facesOut[i] = pe::Vector3<size_t>( vMap[f->vertex[2]->vnum], vMap[f->vertex[1]->vnum], vMap[f->vertex[0]->vnum] );
      f = f->next;
   }

   /*
   std::cout << "o Objekt0" << std::endl;
   for( size_t i = 0; i < verticesOut.size(); ++i )
      std::cout << "v " << verticesOut[i][0] << " " << verticesOut[i][1] << " " << verticesOut[i][2] << std::endl;
   for( size_t i = 0; i < facesOut.size(); ++i )
      std::cout << "f " << facesOut[i][0] + 1 << " " << facesOut[i][1] + 1 << " " << facesOut[i][2] + 1 << std::endl;
   */
...
bool AddOne( tVertex p )
{
   ...
   do {
      vol = VolumeSign( f, p );
      if (debug) fprintf(stderr, "faddr: %6p   paddr: %6p   Vol = %d\n", (void*)f,(void*)p,vol);
      if ( vol < 0 ) {
         f->visible = VISIBLE;
         vis = true;
      }
...
void	PrintVertices( void )
{
   ...
   if (vertices) do {
      fprintf(stderr,"  addr %6p\t", (void*)vertices );
      ...
      fprintf(stderr,"   dup:%5p", (void*)vertices->duplicate );
...
void	PrintEdges( void )
{
   ...
   if (edges) do {
      fprintf( stderr, "  addr: %6p\t", (void*)edges );
      fprintf( stderr, "adj: ");
      for (i=0; i<2; ++i)
         fprintf( stderr, "%6p", (void*)edges->adjface[i] );
...
void	PrintFaces( void )
{
   ...
   if (faces) do {
      fprintf(stderr, "  addr: %10p  ", (void*)faces );
      fprintf(stderr, "  edges:");
      for( i=0; i<3; ++i )
         fprintf(stderr, "%10p ", (void*)faces->edge[i] );
      fprintf(stderr, "  vert:");
      for ( i=0; i<3; ++i)
   ...
   if (faces) do {
      for( i=0; i<3; ++i ) {
         ...
         if ( v != e->endpts[0] && v != e->endpts[1] ) {
            ...
            fprintf(stderr,"  addr: %8p;", (void*)faces );
            fprintf(stderr,"  edges:");
