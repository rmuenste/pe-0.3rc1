...
static const char* const solverRationale =
   "decomposition_geometry's contact and time-integration parameters are tuned for those\n"
 ...
 * Start command i.e.:
 * mpirun -np 2 ./decomposition_geometry --particles 2 1 1 --processes 2 1 1
 */
int main( int argc, char** argv )
{
   ...
   if( !pe::examples::requireIntendedSolver<TargetConfig1, TargetConfig2, TargetConfig3>(
          "decomposition_geometry", intendedSolvers, solverRationale ) ) {
      MPI_Finalize();
      return EXIT_FAILURE;
   }
   ...
   const size_t timesteps ( 2 );  // Total number of time steps
   ...
   // Compare the setup below with the RectilinearGrid setup that is used in examples:
   // - MPIBrazil
   // - MPIChannel
   // - MPIImpact
   std::stringstream ss;
   ...
   simTime.start();
   /////////////////////////////////////////////////////
   // Simulation loop Version 1
   //world->run( timesteps, stepsize );

   /////////////////////////////////////////////////////
   // Simulation loop Version2
