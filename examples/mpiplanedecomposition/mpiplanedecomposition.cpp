...
int main( int argc, char** argv )
{
   ...
   defineLocalDomain(HalfSpace( Vec3(+1,0,0), +center[0]*lpx ));
  ...
  if (myRank == 0) {
   ...
   Vec3 normal =-halfSpaces[center[0]-1].getNormal();
   real disp   = halfSpaces[center[0]-1].getDisplacement();
   defineLocalDomain( 
      intersect(
         HalfSpace(normal, disp),
         halfSpaces[center[0]] 
      )
   );
  }
