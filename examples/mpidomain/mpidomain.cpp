...
static const char* const solverRationale =
   "mpidomain's contact and time-integration parameters are tuned for those\n"
...
int main( int argc, char** argv )
{
   ...
   if( !pe::examples::requireIntendedSolver<TargetConfig2, TargetConfig3>(
          "mpidomain", intendedSolvers, solverRationale ) ) {
      MPI_Finalize();
      return EXIT_FAILURE;
   }
   ...
   const size_t timesteps ( 1 );  // Total number of time steps
...
//   real by =-0.4;
//   real bz = 0.0;
   position[0] = bx + 0.5 * dx + center[0] * dx; 
   position[1] = by + 0.5 * dy + center[1] * dy; 
   position[2] = bz + 0.5 * dz + center[2] * dz; 
   std::cout << "Rank: " << my_cart_rank << "Box: " << (Vec3(position[0], position[1], position[2])) << std::endl;
   std::cout << "Rank: " << my_cart_rank << "Center: " << (Vec3(center[0], center[1], center[2])) << std::endl;
//   int count(0);
   BodyID particle;
   particle = createBox( idx, position, Vec3(0.9 * dx, 0.9 * dy, 0.9 * dz), myMaterial);
