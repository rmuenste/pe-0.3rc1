...
public:
    ...
    //! Body motion for the DNS-drag xyz path: "fixed" (default, D6.1),
    //! "rotationOnly" (translation locked via the linear DOF mask, rotation
    //! free - D6.2 Jeffery), "translationOnly" (all angular DOFs locked via the
    //! angular mask (0,0,0), translation free - D6.4 tier S) or "free" (no DOF
    //! locks at all; gravity/buoyancy through the solver's ellipsoid/sphere
    //! seeding branch - D6.4 G0 / tier I).
    void setParticleMotion(const std::string& motion) {
        if (motion != "fixed" && motion != "rotationOnly" &&
            motion != "translationOnly" && motion != "free")
            throw std::invalid_argument(
                "particleMotion_ must be 'fixed', 'rotationOnly', 'translationOnly' or 'free', got '" +
                motion + "'");
...
private:
    ...
    std::string particleMotion_; //!< DNS-drag xyz-path motion: "fixed" (default), "rotationOnly", "translationOnly" or "free"
