//=================================================================================================
/*!
 *  \file pe/core/rigidbody/InnerMesh.h
 *  \brief Header file for the InnerMesh class
 *
...
#ifndef _PE_CORE_RIGIDBODY_INNERMESH_H_
#define _PE_CORE_RIGIDBODY_INNERMESH_H_
...
#include <pe/core/rigidbody/InnerMeshTrait.h>
 ...
 * mesh. A detailed description of the triangle mesh primitive can be found with the class InnerMesh.
 ...
 */
class InnerMesh : public InnerMeshTrait<Config>
{
protected:
   //**Type definitions****************************************************************************
   typedef InnerMeshTrait<Config>  Parent;  //!< The type of the parent class.
   ...
   /*!\name Constructor */
   //@{
   explicit InnerMesh( id_t sid, id_t uid, const Vec3& gpos,
   ...
   explicit InnerMesh( id_t sid, id_t uid, const Vec3& gpos, const Vec3& rpos, const Quat& q,
                          const Vertices& vertices, const IndicesLists& faceIndices,
                          const Normals& faceNormals,
                          const Normals& vertexNormals, const IndicesLists normalIndices,
                          const TextureCoordinates& texturCoordinates, const IndicesLists& texturIndices,
...
private:
   ...
   /*! \cond PE_INTERNAL */
   friend InnerMeshID createInnerMesh( id_t uid, const Vec3& gpos, const std::string file,
                                             MaterialID material, bool convex, bool visible,
                                             const Vec3& scale,  bool clockwise, bool lefthanded );
   friend InnerMeshID createInnerMesh( id_t uid, const Vec3& gpos, const InnerMeshID source,
                                             MaterialID material, bool visible );
   friend InnerMeshID createInnerMesh( id_t uid, const Vec3& gpos, Vertices vertices,
                                             const IndicesLists& faces, MaterialID material,
                                             bool convex, bool visible );
   friend InnerMeshID instantiateInnerMesh( id_t sid, id_t uid, const Vec3& gpos, const Vec3& rpos,
 ...
 */
inline size_t InnerMesh::size() const
{
 ...
 * \todo Review documentation.
 */
inline size_t InnerMesh::getSmallMeshLimit() const
{
 ...
 * \todo Review documentation.
 */
inline void InnerMesh::setSmallMeshLimit(size_t newLimit)
{
 ...
 */
inline real InnerMesh::getDepth( real px, real py, real pz ) const
{
 ...
 * lies outside the triangle mesh. The returned depth is calculated relative to the closest side of the triangle mesh.
 */
inline real InnerMesh::getDepth( const Vec3& gpos ) const
{
 ...
 */
inline real InnerMesh::getRelDistance( real px, real py, real pz ) const
{
 ...
 * side of the triangle mesh.
 */
inline real InnerMesh::getRelDistance( const Vec3& rpos ) const
{
 ...
 * side of the triangle mesh.
 */
inline real InnerMesh::getDistance( real px, real py, real pz ) const
{
   return -getRelDepth( pointFromWFtoBF( px, py, pz ) );
}
//*************************************************************************************************
 ...
 * side of the triangle mesh.
 */
inline real InnerMesh::getDistance( const Vec3& gpos ) const
{
   return -getRelDepth( pointFromWFtoBF( gpos ) );
}
//*************************************************************************************************
 ...
 * \todo Review documentation.
 */
inline Vec3 InnerMesh::support( const Vec3& d ) const
{
 ...
 * \todo Review documentation.
 */
inline Vec3 InnerMesh::supportContactThreshold( const Vec3& d ) const
{
 ...
 * \todo Review documentation.
 */
inline Vec3 InnerMesh::supportContactThreshold( const Vec3& d, size_t startIndex, size_t* pointIndex ) const
{
 ...
 */
inline ::irr::scene::SMeshBuffer* InnerMesh::getIrrlichtCacheBuffer() const
{
...
//@{
InnerMeshID createInnerMesh( id_t uid, const Vec3& gpos, const std::string file,
...
InnerMeshID createInnerMesh( id_t uid, const Vec3& gpos, const InnerMeshID source,
                                   MaterialID material, bool visible=true);

InnerMeshID createInnerMesh( id_t uid, const Vec3& gpos, Vertices vertices,
                                   const IndicesLists& faces, MaterialID material,
                                   bool convex, bool visible=true );

InnerMeshID instantiateInnerMesh( id_t sid, id_t uid, const Vec3& gpos, const Vec3& rpos,
                                        const Quat& q, const Vertices& vertices,
                                        const IndicesLists& faces, MaterialID material,
...
//@{
std::ostream& operator<<( std::ostream& os, const InnerMesh& m );
std::ostream& operator<<( std::ostream& os, ConstInnerMeshID m );
//@}
//*************************************************************************************************
 ...
 * (InnerMesh,RigidBody). The function traverses the range \f$ [first,last) \f$ of pointers to rigid
 * bodies and counts all pointers to triangle meshes.
 */
template<>
inline size_t polymorphicCount<InnerMesh>( RigidBody *const * first,
                                     RigidBody *const * last )
{
   pe_CONSTRAINT_MUST_BE_STRICTLY_DERIVED_FROM( InnerMesh, RigidBody );
 ...
 *
 * This function is a specialization of the polymorphicCount() function for the type combination
 * (const InnerMesh,RigidBody). The function traverses the range \f$ [first,last) \f$ of pointers to
 * rigid bodies and counts all pointers to triangle meshes.
 */
template<>
inline size_t polymorphicCount<const InnerMesh>( RigidBody *const * first,
                                           RigidBody *const * last )
{
   pe_CONSTRAINT_MUST_BE_STRICTLY_DERIVED_FROM( const InnerMesh, RigidBody );
 ...
 *
 * This function is a specialization of the polymorphicCount() function for the type combination
 * (InnerMesh,const RigidBody). The function traverses the range \f$ [first,last) \f$ of pointers to
 * rigid bodies and counts all pointers to triangle meshes.
 */
template<>
inline size_t polymorphicCount<InnerMesh>( const RigidBody *const * first,
                                     const RigidBody *const * last )
{
   pe_CONSTRAINT_MUST_BE_STRICTLY_DERIVED_FROM( InnerMesh, const RigidBody );
 ...
 *
 * This function is a specialization of the polymorphicCount() function for the type combination
 * (const InnerMesh,const RigidBody). The function traverses the range \f$ [first,last) \f$ of pointers
 * to rigid bodies and counts all pointers to triangle meshes.
 */
template<>
inline size_t polymorphicCount<const InnerMesh>( const RigidBody *const * first,
                                           const RigidBody *const * last )
{
   pe_CONSTRAINT_MUST_BE_STRICTLY_DERIVED_FROM( const InnerMesh, const RigidBody );
 ...
 *
 * This function is a specialization of the polymorphicCount() function for the type combination
 * (InnerMesh,GeomPrimitive). The function traverses the range \f$ [first,last) \f$ of pointers to
 * geometric primitives and counts all pointers to triangle meshes.
 */
template<>
inline size_t polymorphicCount<InnerMesh>( GeomPrimitive *const * first,
                                     GeomPrimitive *const * last )
{
   pe_CONSTRAINT_MUST_BE_STRICTLY_DERIVED_FROM( InnerMesh, GeomPrimitive );
 ...
 *
 * This function is a specialization of the polymorphicCount() function for the type combination
 * (const InnerMesh,GeomPrimitive). The function traverses the range \f$ [first,last) \f$ of pointers
 * to geometric primitives and counts all pointers to triangle meshes.
 */
template<>
inline size_t polymorphicCount<const InnerMesh>( GeomPrimitive *const * first,
                                           GeomPrimitive *const * last )
{
   pe_CONSTRAINT_MUST_BE_STRICTLY_DERIVED_FROM( const InnerMesh, GeomPrimitive );
 ...
 *
 * This function is a specialization of the polymorphicCount() function for the type combination
 * (InnerMesh,const GeomPrimitive). The function traverses the range \f$ [first,last) \f$ of pointers
 * to geometric primitives and counts all pointers to triangle meshes.
 */
template<>
inline size_t polymorphicCount<InnerMesh>( const GeomPrimitive *const * first,
                                     const GeomPrimitive *const * last )
{
   pe_CONSTRAINT_MUST_BE_STRICTLY_DERIVED_FROM( InnerMesh, const GeomPrimitive );
 ...
 *
 * This function is a specialization of the polymorphicCount() function for the type combination
 * (const InnerMesh,const GeomPrimitive). The function traverses the range \f$ [first,last) \f$ of
 * pointers to geometric primitives and counts all pointers to triangle meshes.
 */
template<>
inline size_t polymorphicCount<const InnerMesh>( const GeomPrimitive *const * first,
                                           const GeomPrimitive *const * last )
{
   pe_CONSTRAINT_MUST_BE_STRICTLY_DERIVED_FROM( const InnerMesh, const GeomPrimitive );
 ...
 * (InnerMesh,RigidBody). The function traverses the range \f$ [first,last) \f$ of pointers to
 * rigid bodies until it finds the next pointer to a triangle mesh.
 */
template<>
inline RigidBody *const * polymorphicFind<InnerMesh>( RigidBody *const * first,
                                                RigidBody *const * last )
{
   pe_CONSTRAINT_MUST_BE_STRICTLY_DERIVED_FROM( InnerMesh, RigidBody );
 ...
 */
template<>
inline RigidBody *const * polymorphicFind<const InnerMesh>( RigidBody *const * first,
                                                      RigidBody *const * last )
{
   pe_CONSTRAINT_MUST_BE_STRICTLY_DERIVED_FROM( const InnerMesh, RigidBody );
 ...
 */
template<>
inline const RigidBody *const * polymorphicFind<InnerMesh>( const RigidBody *const * first,
                                                      const RigidBody *const * last )
{
   pe_CONSTRAINT_MUST_BE_STRICTLY_DERIVED_FROM( InnerMesh, const RigidBody );
 ...
 */
template<>
inline const RigidBody *const * polymorphicFind<const InnerMesh>( const RigidBody *const * first,
                                                            const RigidBody *const * last )
{
   pe_CONSTRAINT_MUST_BE_STRICTLY_DERIVED_FROM( const InnerMesh, const RigidBody );
 ...
 */
template<>
inline GeomPrimitive *const * polymorphicFind<InnerMesh>( GeomPrimitive *const * first,
                                                    GeomPrimitive *const * last )
{
   pe_CONSTRAINT_MUST_BE_STRICTLY_DERIVED_FROM( InnerMesh, GeomPrimitive );
 ...
 */
template<>
inline GeomPrimitive *const * polymorphicFind<const InnerMesh>( GeomPrimitive *const * first,
                                                          GeomPrimitive *const * last )
{
   pe_CONSTRAINT_MUST_BE_STRICTLY_DERIVED_FROM( const InnerMesh, GeomPrimitive );
 ...
 */
template<>
inline const GeomPrimitive *const * polymorphicFind<InnerMesh>( const GeomPrimitive *const * first,
                                                          const GeomPrimitive *const * last )
{
   pe_CONSTRAINT_MUST_BE_STRICTLY_DERIVED_FROM( InnerMesh, const GeomPrimitive );
 ...
 */
template<>
inline const GeomPrimitive *const * polymorphicFind<const InnerMesh>( const GeomPrimitive *const * first,
                                                                const GeomPrimitive *const * last )
{
   pe_CONSTRAINT_MUST_BE_STRICTLY_DERIVED_FROM( const InnerMesh, const GeomPrimitive );
