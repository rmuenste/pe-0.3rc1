//=================================================================================================
/*!
 *  \file pe/core/contact/contacttrait/BoxFrictionSolver.h
 *  \brief Specialization of the ContactTrait class template for the box friction solver
 *
...
#ifndef _PE_CORE_CONTACT_CONTACTTRAIT_BOXFRICTIONSOLVER_H_
#define _PE_CORE_CONTACT_CONTACTTRAIT_BOXFRICTIONSOLVER_H_
...
//=================================================================================================
//
//  SPECIALIZATION FOR THE BOX FRICTION CONSTRAINT SOLVER
//
//=================================================================================================

//*************************************************************************************************
/*!\brief Specialization of the ContactTrait class template for the box friction constraint solver.
 * \ingroup core
 *
 * This specialization of the ContactTrait class template adapts all contacts to the requirements
 * of the box friction constraint solver.
 */
...
class ContactTrait< C<CD,FD,BG,response::BoxFrictionSolver> > : public ContactBase
{
protected:
   //**Type definitions****************************************************************************
...
public:
   ...
   void calcMatFL     ( MatN& JLin1, MatN& JAng1, MatN& JLin2, MatN& JAng2 ) const;
   void calcErrorVecFL( VecN& bError )                                       const;
   //@}
   //**********************************************************************************************
};
//*************************************************************************************************
...
//*************************************************************************************************
/*!\brief BoxFrictionSolver specialization of the ContactTrait constructor for vertex/face contacts.
 *
...
template< template<typename> class CD                           // Type of the coarse collision detection algorithm
        ...
        , template< template<typename> class                    // Template signature of the coarse collision detection algorithm
                  ...
                  , template<typename,typename,typename> class  // Template signature of the collision response algorithm
                  > class C >                                   // Type of the configuration
inline ContactTrait< C<CD,FD,BG,response::BoxFrictionSolver> >::ContactTrait( GeomID g1, GeomID g2, const Vec3& gpos,
...
//*************************************************************************************************
/*!\brief BoxFrictionSolver specialization of the ContactTrait constructor for edge/edge contacts.
 *
 * \param g1 The first contacting geometric primitive.
 * \param g2 The second contacting geometric primitive.
 * \param gpos The global position of the contact.
...
template< template<typename> class CD                           // Type of the coarse collision detection algorithm
        ...
        , template< template<typename> class                    // Template signature of the coarse collision detection algorithm
                  ...
                  , template<typename,typename,typename> class  // Template signature of the collision response algorithm
                  > class C >                                   // Type of the configuration
inline ContactTrait< C<CD,FD,BG,response::BoxFrictionSolver> >::ContactTrait( GeomID g1, GeomID g2, const Vec3& gpos, const Vec3& normal,
                                                                              const Vec3& e1, const Vec3& e2, real dist )
...
//*************************************************************************************************
/*!\brief BoxFrictionSolver specialization of the ContactTrait destructor.
 */
template< template<typename> class CD                           // Type of the coarse collision detection algorithm
        , typename FD                                           // Type of the fine collision detection algorithm
        , template<typename> class BG                           // Type of the batch generation algorithm
        , template< template<typename> class                    // Template signature of the coarse collision detection algorithm
                  ...
                  , template<typename,typename,typename> class  // Template signature of the collision response algorithm
                  > class C >                                   // Type of the configuration
inline ContactTrait< C<CD,FD,BG,response::BoxFrictionSolver> >::~ContactTrait()
{}
//*************************************************************************************************
...
template< template<typename> class CD                           // Type of the coarse collision detection algorithm
        ...
        , template< template<typename> class                    // Template signature of the coarse collision detection algorithm
                  ...
                  , template<typename,typename,typename> class  // Template signature of the collision response algorithm
                  > class C >                                   // Type of the configuration
inline size_t ContactTrait< C<CD,FD,BG,response::BoxFrictionSolver> >::getRows() const
{
   return size_t(3);
}
//*************************************************************************************************
...
//*************************************************************************************************
/*!\brief Calculating the matrices for a box friction contact.
 *
 ...
 * \return void
 *
 * This function calculates the Jacobian matrices for a box friction contact.
 */
template< template<typename> class CD                           // Type of the coarse collision detection algorithm
        , typename FD                                           // Type of the fine collision detection algorithm
        , template<typename> class BG                           // Type of the batch generation algorithm
        , template< template<typename> class                    // Template signature of the coarse collision detection algorithm
                  ...
                  , template<typename,typename,typename> class  // Template signature of the collision response algorithm
                  > class C >                                   // Type of the configuration
void ContactTrait< C<CD,FD,BG,response::BoxFrictionSolver> >::calcMat( MatN& JLin1, MatN& JAng1, MatN& JLin2, MatN& JAng2 ) const
{
...
//*************************************************************************************************
/*!\brief Calculating the error vector for a box friction contact.
 *
 * \param bError The velocity error correcting vector.
 * \return void
 *
 * This function calculates the velocity error correcting term for a box friction contact.
 */
template< template<typename> class CD                           // Type of the coarse collision detection algorithm
        , typename FD                                           // Type of the fine collision detection algorithm
        , template<typename> class BG                           // Type of the batch generation algorithm
        , template< template<typename> class                    // Template signature of the coarse collision detection algorithm
                  ...
                  , template<typename,typename,typename> class  // Template signature of the collision response algorithm
                  > class C >                                   // Type of the configuration
void ContactTrait< C<CD,FD,BG,response::BoxFrictionSolver> >::calcErrorVec( VecN& bError ) const
{
   real bounce( 0 );
...
//*************************************************************************************************
/*!\brief Calculating the matrices for a frictionless contact.
 *
 * \param JLin1 The linear Jacobian matrix for body 1.
 * \param JAng1 The angular Jacobian matrix for body 1.
 * \param JLin2 The linear Jacobian matrix for body 2.
 ...
 * \return void
 *
 * This function calculates the Jacobian matrices for a frictionless contact.
 */
template< template<typename> class CD                           // Type of the coarse collision detection algorithm
        , typename FD                                           // Type of the fine collision detection algorithm
        , template<typename> class BG                           // Type of the batch generation algorithm
        , template< template<typename> class                    // Template signature of the coarse collision detection algorithm
                  ...
                  , template<typename,typename,typename> class  // Template signature of the collision response algorithm
                  > class C >                                   // Type of the configuration
void ContactTrait< C<CD,FD,BG,response::BoxFrictionSolver> >::calcMatFL( MatN& JLin1, MatN& JAng1, MatN& JLin2, MatN& JAng2 ) const
{
   const Vec3 r1( b1_->getPosition() );
   const Vec3 r2( b2_->getPosition() );
   ...
   const real initLin1[1][3] = { {  normal_[0],  normal_[1],  normal_[2] } };
   JLin1 = initLin1;

   const real initAng1[1][3] = { { -normal_[1] * a1[2] + normal_[2] * a1[1],
                                    normal_[0] * a1[2] - normal_[2] * a1[0],
                                   -normal_[0] * a1[1] + normal_[1] * a1[0] } };
   JAng1 = initAng1;

   const real initLin2[1][3] = { { -normal_[0], -normal_[1], -normal_[2] } };
   JLin2 = initLin2;

   const real initAng2[1][3] = { {  normal_[1] * a2[2] - normal_[2] * a2[1],
                                   -normal_[0] * a2[2] + normal_[2] * a2[0],
                                    normal_[0] * a2[1] - normal_[1] * a2[0] } };
   JAng2 = initAng2;
}
...
//*************************************************************************************************
/*!\brief Calculating the error vector for a frictionless contact.
 *
 * \param bError The velocity error correcting vector.
 * \return void
 *
 * This function calculates the velocity error correcting term for a frictionless contact.
 */
template< template<typename> class CD                           // Type of the coarse collision detection algorithm
        , typename FD                                           // Type of the fine collision detection algorithm
        , template<typename> class BG                           // Type of the batch generation algorithm
        , template< template<typename> class                    // Template signature of the coarse collision detection algorithm
                  ...
                  , template<typename,typename,typename> class  // Template signature of the collision response algorithm
                  > class C >                                   // Type of the configuration
void ContactTrait< C <CD,FD,BG,response::BoxFrictionSolver> >::calcErrorVecFL( VecN& bError) const
{
   real bounce( 0 );
   ...
   const real init[1] = { error - bounce };
   bError = init;
}
//*************************************************************************************************
