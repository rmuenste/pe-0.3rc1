//=================================================================================================
/*!
 *  \file pe/core/response/mpidecoder/DEMSolverObsolete.h
 *  \brief Specialization of the MPIDecoder class template for the discrete element solver
 *
...
#ifndef _PE_CORE_RESPONSE_MPIDECODER_DEMSOLVEROBSOLETE_H_
#define _PE_CORE_RESPONSE_MPIDECODER_DEMSOLVEROBSOLETE_H_
...
#include <pe/core/response/mpidecoder/Default.h>
...
class MPIDecoder< C<CD,FD,BG,response::DEMSolverObsolete> >
{
public:
   //**Type definitions****************************************************************************
...
public:
   ...
   //@{
   template< typename RB > inline BodyID         decodeBody      ( RB& buffer );
...
private:
   ...
   /*!\name Decode functions */
   //@{
   template< typename RB > BodyID         decodeSuperBody( RB& buffer );
   ...
   template< typename RB > BodyID         decodeUpdate   ( RB& buffer );
...
//*************************************************************************************************
/*!\brief Constructor of the MPIDecoder specialization for the DEM collision system.
 *
...
template< template<typename> class CD                           // Type of the coarse collision detection algorithm
        ...
        , template< template<typename> class                    // Template signature of the coarse collision detection algorithm
                  ...
                  , template<typename,typename,typename> class  // Template signature of the collision response algorithm
                  > class C >                                   // Type of the configuration
inline MPIDecoder< C<CD,FD,BG,response::DEMSolverObsolete> >::MPIDecoder( BS& bodystorage, AS& attachablestorage )
...
inline typename MPIDecoder< C<CD,FD,BG,response::DEMSolverObsolete> >::BodyID
   MPIDecoder< C<CD,FD,BG,response::DEMSolverObsolete> >::decodeBody( RB& buffer )
{
...
template< template<typename> class CD                           // Type of the coarse collision detection algorithm
        ...
        , template< template<typename> class                    // Template signature of the coarse collision detection algorithm
                  ...
                  > class C >                                   // Type of the configuration
template< typename RB >                                         // Type of the receive buffer
typename MPIDecoder< C<CD,FD,BG,response::DEMSolverObsolete> >::AttachableData
   MPIDecoder< C<CD,FD,BG,response::DEMSolverObsolete> >::decodeAttachable( RB& buffer ) const
{
   pe_INTERNAL_ASSERT( !buffer.isEmpty()           , "Empty receive buffer detected"  );
...
template< template<typename> class CD                           // Type of the coarse collision detection algorithm
        ...
        , template< template<typename> class                    // Template signature of the coarse collision detection algorithm
                  ...
                  > class C >                                   // Type of the configuration
template< typename RB >                                         // Type of the receive buffer
typename MPIDecoder< C<CD,FD,BG,response::DEMSolverObsolete> >::BodyID
   MPIDecoder< C<CD,FD,BG,response::DEMSolverObsolete> >::decodeSuperBody( RB& buffer )
{
   byte tag;
...
template< typename RB >                                         // Type of the receive buffer
typename MPIDecoder< C<CD,FD,BG,response::DEMSolverObsolete> >::BodyID
   MPIDecoder< C<CD,FD,BG,response::DEMSolverObsolete> >::decodeSubBody( RB& buffer ) const
{
   byte tag;
...
template< typename RB >                                         // Type of the receive buffer
typename MPIDecoder< C<CD,FD,BG,response::DEMSolverObsolete> >::BodyID
   MPIDecoder< C<CD,FD,BG,response::DEMSolverObsolete> >::decodeSphere( RB& buffer, bool superordinate ) const
{
   bool visible, fixed;
   id_t sid, uid;
   MaterialID material;
   real radius;
   Vec3 gpos, rpos, v, w, vdot, wdot;
   Quat q;
   ...
   if( superordinate ) {
      ...
      unmarshal( buffer, vdot );   // Decoding the linear acceleration
      unmarshal( buffer, wdot );   // Decoding the angular acceleration
   }
   ...
   if( superordinate ) {
      ...
      body->setLinearAcc ( vdot );
      body->setAngularAcc( wdot );
   }
...
template< typename RB >                                         // Type of the receive buffer
typename MPIDecoder< C<CD,FD,BG,response::DEMSolverObsolete> >::BodyID
   MPIDecoder< C<CD,FD,BG,response::DEMSolverObsolete> >::decodeBox( RB& buffer, bool superordinate ) const
{
   bool visible, fixed;
   id_t sid, uid;
   MaterialID material;
   Vec3 lengths, gpos, rpos, v, w, vdot, wdot;
   Quat q;
   BodyID body;
...
template< typename RB >                                         // Type of the receive buffer
typename MPIDecoder< C<CD,FD,BG,response::DEMSolverObsolete> >::BodyID
   MPIDecoder< C<CD,FD,BG,response::DEMSolverObsolete> >::decodeCapsule( RB& buffer, bool superordinate ) const
{
   bool visible, fixed;
   id_t sid, uid;
   MaterialID material;
...
template< typename RB >                                         // Type of the receive buffer
typename MPIDecoder< C<CD,FD,BG,response::DEMSolverObsolete> >::BodyID
   MPIDecoder< C<CD,FD,BG,response::DEMSolverObsolete> >::decodeCylinder( RB& buffer, bool superordinate ) const
{
   bool visible, fixed;
   id_t sid, uid;
   MaterialID material;
...
template< typename RB >                                         // Type of the receive buffer
typename MPIDecoder< C<CD,FD,BG,response::DEMSolverObsolete> >::BodyID
   MPIDecoder< C<CD,FD,BG,response::DEMSolverObsolete> >::decodeUnion( RB& buffer, bool superordinate ) const
{
   bool visible, fixed;
   id_t sid, uid;
   size_t size;
...
template< typename RB >                                         // Type of the receive buffer
typename MPIDecoder< C<CD,FD,BG,response::DEMSolverObsolete> >::BodyID
   MPIDecoder< C<CD,FD,BG,response::DEMSolverObsolete> >::decodeUpdate( RB& buffer )
{
   bool visible;
   id_t sid;
   Vec3 gpos, v, w, vdot, wdot;
   Quat q;
...
template< typename RB >                                         // Type of the receive buffer
typename MPIDecoder< C<CD,FD,BG,response::DEMSolverObsolete> >::AttachableData
   MPIDecoder< C<CD,FD,BG,response::DEMSolverObsolete> >::decodeGravity( RB& buffer ) const
{
   id_t sid, body;
   Vec3 gravity;
...
template< typename RB >                                         // Type of the receive buffer
typename MPIDecoder< C<CD,FD,BG,response::DEMSolverObsolete> >::AttachableData
   MPIDecoder< C<CD,FD,BG,response::DEMSolverObsolete> >::decodeSpring( RB& buffer ) const
{
   bool visible;
...
template< template<typename> class CD                           // Type of the coarse collision detection algorithm
        ...
        , template< template<typename> class                    // Template signature of the coarse collision detection algorithm
                  ...
                  > class C >                                   // Type of the configuration
template< typename RB >                                         // Type of the receive buffer
inline MPIEntity MPIDecoder< C<CD,FD,BG,response::DEMSolverObsolete> >::peek( const RB& buffer ) const
{
   pe_INTERNAL_ASSERT( !buffer.isEmpty(), "Empty receive buffer detected" );
