...
#ifndef _PE_UTIL_TIMING_TIMER_H_
#define _PE_UTIL_TIMING_TIMER_H_
...
//*************************************************************************************************
/*!\brief Ending a single time measurement and starting a new one.
 *
 * \return void
 *
 * This function ends the currently running time measurement, performs the necessary
 * statistical calculations and starts the next time measurement without delay.
 */
template< typename TP >  // Timing policy
inline void Timer<TP>::lap()
{
   end();
   start_ = end_;
}
