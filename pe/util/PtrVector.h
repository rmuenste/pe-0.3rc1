...
#ifndef _PE_UTIL_PTRVECTOR_H_
#define _PE_UTIL_PTRVECTOR_H_
 ...
 * problems in the context of the \b pe, where vectors to pointers are used both internally
 * (including proper resource management) and outside by the user (without any resource
 * management).\n
 * Since both \a std::vector and \a boost::ptr_vector are not suitable data structures in the
 * context of the \a pe, the \b pe provides a special vector container for pointers, which is
 * a cross of the functionalities of the \a std::vector and \a ptr_vector. The \b pe PtrVector
 ...
 * However, it perfectly fits the requirements of the \b pe: PtrVector can be used internally
 * to store pointers to dynamically allocated objects and resources within RAII classes, and
 * outside the \b pe by a user as storage for handles to resources that are managed elsewhere.
 * In contrast to the \a boost::ptr_vector, the PtrVector provides full access to the contained
 * pointers (in the \b pe context handles), but its iterators work similar to the \a ptr_vector
 ...
 * that objects are stored instead of pointers:
   ...
   \code
   typedef pe::PtrVector<double>  Doubles;
 ...
 * convention of the \b pe physics engine (i.e. pushBack instead of push_back). In addition,
 ...
 * argument: instead of the pointer type, the \b pe pointer vector is only provided with the
 * type of the underlying objects:
   ...
   // pe style:
   pe::PtrVector<double> vector;
   \endcode

 * Additionally, the \b pe pointer vector offers some limited possibilities to configure the
 ...
 * iterate over all objects of a specific type: the CastIterator and ConstCastIterator.
   ...
   void f( pe::PtrVector<A>& vector )
   {
      pe::PtrVector<A>::CastIterator<B> begin = vector.begin<B>();
      pe::PtrVector<A>::CastIterator<B> end   = vector.end<B>();
   ...
   void f( const pe::PtrVector<A>& vector )
   {
      pe::PtrVector<A>::ConstCastIterator<B> begin = vector.begin<B>();
      pe::PtrVector<A>::ConstCastIterator<B> end   = vector.end<B>();
 ...
 * type within the container, the following approach could be taken:
   ...
   class Container
   {
    public:
      template< typename C >
      struct CastIterator : public pe::PtrVector<A>::CastIterator<C>
      {
         CastIterator( const pe::PtrVector<A>::CastIterator<C>& it )
            : pe::PtrVector<A>::CastIterator<C>( it )  // Initializing the base class
         {}
      };
 ...
 *
 *
 * \section definitions Predefined pointer vectors
 *
 * In order to guarantee an easy and efficient work with the physics engine, the engine defines
 * several new vector types. For these types, no adaption of the growth and deleting behavior is
 * necessary. Among these types are for example
 *
 *  - pe::SphereVector : pointer vector for spheres.
 *  - pe::BoxVector : pointer vector for boxes.
 *  - pe::CapsuleVector : pointer vector for capsules.
 *  - pe::PlaneVector : pointer vector for planes.
 *  - pe::UnionVector : pointer vector for unions.
 */
 ...
 * compile time error.
   ...
   typedef pe::PtrVector<A> AVector;
   AVector vector;
...
{
   pe_USER_ASSERT( size_ > 0, "Pointer vector is empty, invalid access to the front element" );
   return *begin_;
}
//*************************************************************************************************
...
{
   pe_USER_ASSERT( size_ > 0, "Pointer vector is empty, invalid access to the back element" );
   return *(end_-1);
}
//*************************************************************************************************
...
template< typename T    // Type
        ...
        , typename G >  // Growth policy
template< typename C >  // Cast type
inline typename PtrVector<T,D,G>::pe_TEMPLATE CastIterator<C> PtrVector<T,D,G>::begin()
{
...
template< typename T    // Type
        ...
        , typename G >  // Growth policy
template< typename C >  // Cast type
inline typename PtrVector<T,D,G>::pe_TEMPLATE ConstCastIterator<C> PtrVector<T,D,G>::begin() const
{
...
template< typename T    // Type
        ...
        , typename G >  // Growth policy
template< typename C >  // Cast type
inline typename PtrVector<T,D,G>::pe_TEMPLATE CastIterator<C> PtrVector<T,D,G>::end()
{
...
template< typename T    // Type
        ...
        , typename G >  // Growth policy
template< typename C >  // Cast type
inline typename PtrVector<T,D,G>::pe_TEMPLATE ConstCastIterator<C> PtrVector<T,D,G>::end() const
{
...
template< typename T    // Type
        ...
        , typename G >  // Growth policy
template< typename C >  // Cast type
inline typename PtrVector<T,D,G>::pe_TEMPLATE CastIterator<C>
 ...
 * is a type derived from \a T.
   ...
   pe::PtrVector<A>::CastIterator<B> begin;
   pe::PtrVector<A>::CastIterator<B> end;
...
{
   pe_CONSTRAINT_MUST_BE_STRICTLY_DERIVED_FROM( C, T );
   pe_CONSTRAINT_MUST_BE_CONVERTIBLE( Other*, C* );
}
//*************************************************************************************************
...
template< typename T    // Type
        ...
        , typename G >  // Growth policy
template< typename C >  // Cast type
inline typename PtrVector<T,D,G>::pe_TEMPLATE CastIterator<C>::PointerType
...
template< typename T    // Type
        ...
        , typename G >  // Growth policy
template< typename C >  // Cast type
inline const typename PtrVector<T,D,G>::pe_TEMPLATE CastIterator<C>::IteratorType&
 ...
 * constant vectors.
   ...
   pe::PtrVector<A>::ConstCastIterator<B> begin;
   pe::PtrVector<A>::ConstCastIterator<B> end;
...
public:
   ...
   template< typename Other >
   inline ConstCastIterator( const typename PtrVector<T,D,G>::pe_TEMPLATE CastIterator<Other>& it );
...
template< typename C >      // Cast type
template< typename Other >  // The foreign cast iterator type
inline PtrVector<T,D,G>::ConstCastIterator<C>::ConstCastIterator( const typename PtrVector<T,D,G>::pe_TEMPLATE CastIterator<Other>& it )
   : cur_( it.base() )  // Pointer to the current memory location
   , end_( it.stop() )  // Pointer to the element one past the last element in the element range
{
   pe_CONSTRAINT_MUST_BE_STRICTLY_DERIVED_FROM( C, T );
...
template< typename T    // Type
        ...
        , typename G >  // Growth policy
template< typename C >  // Cast type
inline typename PtrVector<T,D,G>::pe_TEMPLATE ConstCastIterator<C>&
...
template< typename T    // Type
        ...
        , typename G >  // Growth policy
template< typename C >  // Cast type
inline typename PtrVector<T,D,G>::pe_TEMPLATE ConstCastIterator<C>::PointerType
...
template< typename T    // Type
        ...
        , typename G >  // Growth policy
template< typename C >  // Cast type
inline const typename PtrVector<T,D,G>::pe_TEMPLATE ConstCastIterator<C>::IteratorType&
