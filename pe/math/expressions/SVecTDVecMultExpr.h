//=================================================================================================
/*!
 *  \file pe/math/expressions/SVecTDVecMultExpr.h
...
#ifndef _PE_MATH_EXPRESSIONS_SVECTDVECMULTEXPR_H_
#define _PE_MATH_EXPRESSIONS_SVECTDVECMULTEXPR_H_
   ...
   operator*( const SparseVector<T1,false>& lhs, const DenseVector<T2,true>& rhs )
{
   ...
   typedef typename X1::ConstIterator            ConstIterator;  // Iterator type of the left-hand side sparse vector expression

   pe_CONSTRAINT_MUST_BE_SPARSE_VECTOR_TYPE( T1 );
   pe_CONSTRAINT_MUST_BE_DENSE_VECTOR_TYPE ( T2 );
   ...
   const ConstIterator end( left.end() );

   // Estimation of the number of nonzeros per row
   std::vector<size_t> capacities( m );
   for( ConstIterator element=left.begin(); element!=end; ++element )
      if( !isDefault( element->value() ) )
         capacities[ element->index() ] = n;
   ...
   for( ConstIterator element=left.begin(); element!=end; ++element ) {
      if( !isDefault( element->value() ) ) {
         for( size_t i=0; i<n; ++i ) {
            if( !isDefault( right[i] ) )
               A.append( element->index(), i, element->value() * right[i] );
         }
      }
   }
