...
#ifndef _PE_MATH_QUATERNION_H_
#define _PE_MATH_QUATERNION_H_
...
template< typename Type >       class RotationMatrix;
template< typename Type, bool > class Vector3;
...
public:
   ...
   explicit inline Quaternion( Vector3<Axis,false> axis, Type angle );

   explicit inline Quaternion( Type xangle, Type yangle, Type zangle );

   template< typename Other >
   explicit inline Quaternion( const Vector3<Other,false>& euler );
   ...
   //@{
                              ...
                              inline const Vector3<Type, false> getAxis()          const;
   ...
   */
   ...
   inline const Vector3< typename MathTrait<Type,Other>::MultType, false >
      rotate( const Vector3<Other,TF>& v ) const;

   template< typename Other >
   inline const Matrix3x3< typename MathTrait<Type,Other>::MultType >
      rotate( const Matrix3x3<Other>& m ) const;

   template< typename Other >
   inline const Matrix3x3< typename MathTrait<Type,Other>::MultType >
      diagRotate( const Matrix3x3<Other>& m ) const;

   template< typename Other >
   inline typename MathTrait<Type,Other>::HighType
      calcAngle( const Vector3<Other,false>& axis ) const;
   //@}
   //**********************************************************************************************
...
inline Quaternion<Type>::Quaternion( Type r, Type i, Type j, Type k )
{
   ...
   pe_USER_ASSERT( std::fabs( r*r + i*i + j*j + k*k - Type(1) ) < Type(accuracy), "Invalid quaternion parameters" );
}
//*************************************************************************************************
...
inline Quaternion<Type>::Quaternion( Vector3<Axis,false> axis, Type angle )
{
   ...
   pe_USER_ASSERT( axis.sqrLength() > Axis(0), "Invalid rotation axis" );
...
inline Quaternion<Type>::Quaternion( const Vector3<Other,false>& euler )
{
   reset();
   rotateX( euler[0] );
   rotateY( euler[1] );
...
{
   pe_USER_ASSERT( index < 4, "Invalid quaternion access index" );
   return v_[index];
}
//*************************************************************************************************
...
{
   pe_USER_ASSERT( std::fabs( r*r + i*i + j*j + k*k - Type(1) ) < Type(1E-8), "Invalid quaternion parameters" );
   v_[0] = r;
   v_[1] = i;
   v_[2] = j;
   v_[3] = k;
 ...
 */
template< typename Type >
inline const Vector3<Type, false> Quaternion<Type>::getAxis() const
{
   Type s( std::sqrt( 1-v_[0]*v_[0] ) );
   if (s < epsilon)
      return Vector3<Type, false>( 1, 0, 0 );
   else
      return Vector3<Type, false>( v_[1] / s, v_[2] / s, v_[3] / s );
}
//*************************************************************************************************
...
inline const Vector3< typename MathTrait<Type,Other>::MultType, false >
   Quaternion<Type>::rotate( const Vector3<Other,TF>& v ) const
{
   ...
   return Vector3<MT,TF>( v_[0]*x + v_[1]*w + v_[2]*z - v_[3]*y,
   ...
   return Vector3<MT,TF>( ( v[0]*( v0 + v1 - v2 - v3 ) + 2.0*v_[0]*( v_[2]*v[2] - v_[3]*v[1] ) + 2.0*v_[1]*( v_[2]*v[1] + v_[3]*v[2] ) ),
...
inline const Matrix3x3< typename MathTrait<Type,Other>::MultType >
   Quaternion<Type>::rotate( const Matrix3x3<Other>& m ) const
{
   typedef typename MathTrait<Type,Other>::MultType  MT;
   const Matrix3x3<MT> R( this->toRotationMatrix() );
   return R.rotate( m );
}
//*************************************************************************************************
...
inline const Matrix3x3< typename MathTrait<Type,Other>::MultType >
   Quaternion<Type>::diagRotate( const Matrix3x3<Other>& m ) const
{
   typedef typename MathTrait<Type,Other>::MultType  MT;
   const Matrix3x3<MT> R( this->toRotationMatrix() );
...
inline typename MathTrait<Type,Other>::HighType
   Quaternion<Type>::calcAngle( const Vector3<Other,false>& axis ) const
{
   typedef typename MathTrait<Type,Other>::HighType  High;

   const Vector3<High,true> u( v_[1], v_[2], v_[3] );
